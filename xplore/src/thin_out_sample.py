...
def thin_out_log(input_file, output_file, m):
    # Read sample from input_file and write into output_file every mth tree
    count = 0
    o_file = open(output_file, "w")
    with open(input_file) as i_file:
        for line in i_file:
            if count == 0:
                ...
                o_file.write(line)
                if re.match("0", line):
                    # First line containing information for a sample -- update counter
                    count += 1
            else:
                count +=1
                if (count-1) % m == 0:
...
if __name__ == '__main__':
    ...
    thin_out_log("../simulations/posterior/primates/primate-mtDNA.log", "../simulations/posterior/primates/primate-mtDNA_small.log", 10000)
